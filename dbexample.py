...
DB_EXC=None
	def setup():
 		...
 		(g)adfly
 		(s)qlite
         enter your choice:''').strip().lower()[0]]
 	def connect(db,dbname):
 		...
 		if not os.path.isdir(dbdir):
 			...
 			except _mysql_exceptions.OperationlError,e:
 				cxn=MySQLdb.connect(user='root')
 			try:
 				cxn.query('DROP DATEBASE %s'% dbname)
 			except DB_EXC.OperationlError,e:
 				pass
	...
	def main():
		...
		cur = cxn.cursor()
 		...
 		print '\n*** Randomly moving folks',fr, to, num = update(cur)
		print 'from one group (%d) to another (%d)' % (fr, to)
 		print '\t(%d users moved)' % numdbDump(cur)

 		print '\n*** Randomly choosing group',rm, num = delete(cur)
 		print '(%d) to delete' % rm
 		print '\t(%d users removed)' % numdbDump(cur)
